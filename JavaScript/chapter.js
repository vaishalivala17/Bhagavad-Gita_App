...
document.getElementById("chapter-title").textContent = `${chapterNumber}. ${chapterName}`;
...
async function loadShlokas() {
  try {
    const res = await fetch(`https://vedicscriptures.github.io/slok/${chapter.chapterNumber}.json`);
    const data = await res.json();

    // Normalize structure
    if (Array.isArray(data)) {
      allShlokas = data.filter(
      ...
      allShlokas = Object.values(data).filter(
        s => s && s.slok && s.slok.trim() !== ""
      );
    }
    ...
    if (allShlokas.length === 0) {
      document.getElementById("shloka-container").innerHTML =
